...
class DateParser:
    ...
    def get_date_list(self, content):
        ...
        for i in date_values:
            for j in i:
                ss = re.sub('\s+(?!$)',' ',j) #正向否定预查不查找$(结束)前面的\s+
                if ss != "":
                    date_list.append(ss)

        if len(date_list) > 0:
            date_value = date_list[0]#一定匹配第一个
            if find_flag:
...
def out_date(date_value):
    date = ''
    obj = DateParser()
    date_value = obj.get_date_list(date_value)
    date = toolsutil.norm_date_time(date_value)
    return date


if __name__ == "__main__":
    obj = DateParser()
    # date_value = u"公告日期：2016-07-14 下午3:33 "
    # date_value = "null"
    # date_value = 1111
    # date_value = "二〇一六年十二月十二日"
    date_value = u"发布时间：2017年8月18日 11:57"
    # da = obj.get_date_list(date_value)
    # date = toolsutil.norm_date_time(da)
    # print date
    print out_date(date_value)